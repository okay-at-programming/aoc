...
for wire, data in enumerate(input):
  data = data.split(',')
  point = 0,0
  step = 1
  for inst in data:
    ...
    for i in range(dist):
      ...
      if point in visited and wire != visited[point][0] and point not in crosses:
        crosses[point] = step + visited[point][1]
      if point not in visited:
        visited[point] = wire,step
      step += 1

shortest = -1
shortpoint = None
for key, dist in crosses.items():
